...
const BarcodeScanner: React.FC<BarcodeScannerProps> = ({ onScan, onClose }) => {
  ...
  const [zoomLevel, setZoomLevel] = useState(2.0); // Default x2 zoom
    ...
    <div style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', display: 'flex', flexDirection: 'column', zIndex: 1, pointerEvents: 'none' }}>
      ...
      <div style={{ padding: '25px 20px', background: 'linear-gradient(rgba(0,0,0,0.8), transparent)', backdropFilter: 'blur(10px)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', pointerEvents: 'auto' }}>
          <div>
            ...
            <p style={{ margin: 0, fontSize: '0.65rem', opacity: 0.6, letterSpacing: '1px' }}>ZOOM x2 ACTIVADO / HD</p>
          </div>
      ...
      <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative' }}>
          ...
          <div style={{ position: 'absolute', bottom: 'calc(50% - 140px)', background: 'rgba(0,255,136,0.1)', border: '1px solid rgba(0,255,136,0.5)', padding: '5px 12px', borderRadius: '12px', animation: 'pulse-zoom 2s infinite' }}>
            <span style={{ fontSize: '0.7rem', color: '#00ff88', fontWeight: 900 }}>DIGITAL ZOOM x2.0</span>
          </div>

          {isStarting && <div style={{ position: 'absolute', bottom: 'calc(50% - 180px)', color: '#00ff88', fontWeight: 900, fontSize: '0.8rem', textShadow: '0 0 10px #000' }}>CONFIGURANDO LENTE...</div>}
