                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '15px' }}>
                            ...
                            <div>
                                ...
                                <select value={editingProduct.categoryId} onChange={e => setEditingProduct({ ...editingProduct, categoryId: e.target.value })} style={{ borderRadius: '15px', padding: '15px', border: '1px solid rgba(0,0,0,0.08)', background: 'var(--bg)' }}>
                                    {globalCategories.slice(1).map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
                                </select>
                            </div>
                        </div>
                        <div>
                            <label style={{ fontWeight: 700, fontSize: '0.8rem', marginBottom: '10px', display: 'block' }}>Etiquetas Destacadas</label>
                            <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                                {globalTags.map(tag => (
                                    <button
                                        key={tag}
                                        ...
                                        style={{
                                            ...
                                            padding: '8px 18px', borderRadius: '30px', fontSize: '0.75rem', fontWeight: 700, border: '1px solid rgba(0,0,0,0.05)', transition: 'var(--transition)'
                                        }}>
                                        {tag}
                                    </button>
                                ))}
